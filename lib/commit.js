...
module.exports = async (branch, commitId) => {
  ...
  const { needBuild } = await prompt({ ...config.needBuild, default: defaultMessage.needBuild })
  try {
    ...
    setCache({ ...defaultMessage, ciType, ciMessage, needBuild })
    await doWidthBr(branch, brResult[0], brResult[0], needBuild)
  } catch(e) {
    spinner.fail('提交异常')
    throw e
  }
