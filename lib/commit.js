...
module.exports = async (branch, commitId) => {
  ...
  if (config.mainBr.includes(branch || currentBr)) {
    ...
    const buildQuaRes = await prompt({
      ...config.needBuild,
      default: defaultMessage.script &&
        defaultMessage.script[projectName] &&
        defaultMessage.script[projectName].isNeedBuild
    })
  ...
  const { ciType } = await prompt({
    ...
    default: defaultMessage.script &&
      defaultMessage.script[projectName] &&
      defaultMessage.script[projectName].ciType
  })
  const { ciMessage } = await prompt({
    ...config.ciMessage,
    default: defaultMessage.script &&
      defaultMessage.script[projectName] &&
      defaultMessage.script[projectName].ciMessage
  })
  ...
  try {
    ...
    // 写入缓存
    await setProjectScript(projectName, { ciType, ciMessage, isNeedBuild }, defaultMessage)
    // 根据当前分支做操作
