...
module.exports = async (branch, commitId) => {
  ...
  const { ciType } = await prompt({
	  ...config.ciType,
	  default: defaultMessage.script && defaultMessage.script[projectName] && defaultMessage.script[projectName].ciType
  })
  const { ciMessage } = await prompt({
	  ...config.ciMessage,
	  default: defaultMessage.script && defaultMessage.script[projectName] && defaultMessage.script[projectName].ciMessage
  })
  ...
  try {
    ...
    spinner.succeed(`提交完成[${brResult[1]}]\n  ${message.match(/"(.*)"/)[1]}`)
	  await setProjectScript(projectName, { ciType, ciMessage }, defaultMessage)
