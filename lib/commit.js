...
module.exports = async (branch, commitId) => {
  ...
  const { ciType } = await prompt({ ...config.ciType, default: defaultMessage.script.ciType })
  const { ciMessage } = await prompt({ ...config.ciMessage, default: defaultMessage.script.ciMessage })
