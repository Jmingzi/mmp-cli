    ...
    http.get('https://registry.npm.taobao.org/mmp-cli/latest', res => {
      ...
      res.on('end', async () => {
        try {
          ...
          const localVersion = await runCmd('mmp -V').catch(() => {
            noticeUpdate(`v1.0.0`, `v${parsedData.version}`)
            setCheckTs(cache)
            resolve()
          })
          if (localVersion && localVersion.trim().substring(1) !== parsedData.version) {
            noticeUpdate(localVersion, `v${parsedData.version}`)
          }
          setCheckTs(cache)
          resolve()
        } catch (e) {
